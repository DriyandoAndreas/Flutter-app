...
class _ListKomunikasiTahfidzState extends State<ListKomunikasiTahfidz>  with WidgetsBindingObserver {

   final _scrollController = ScrollController();
  late SqLiteHelper _sqLiteHelper;
  late List<SqliteUserModel> _users = [];
  late List<KomunikasiTahfidzModel> _listTahfidz = [];
  int _limit = 20;
  ...
  Future<void> _loadList(int limit) async {
    ...
    try {
      ...
      if (id != null && tokenss != null) {
        final paginatedList = await KomunikasiService().getTahfidz(
          id: id,
          tokenss: tokenss.substring(0, 30),
          limit: limit,
        );
        setState(() {
          _listTahfidz = paginatedList;
          _isLoading = false;
        });
      } else {
  ...
  Widget buildList() {
    if (_isLoading && _listTahfidz.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    } else if (_listTahfidz.isEmpty) {
      return const Center(
      ...
      return ListView.builder(
        shrinkWrap: true,
        itemCount: _listTahfidz.length + 1,
        controller: _scrollController,
        itemBuilder: (context, index) {
          if (index < _listTahfidz.length) {
            final listTahfidz = _listTahfidz[index];
            final currentUser = _users.isNotEmpty ? _users.first : null;
            final isAuthorized = currentUser?.siskokode == listTahfidz.kodePegawai;
            return Column(
              children: [
                isAuthorized
                    ? Slidable(
                        endActionPane: ActionPane(
                          ...
                          children: [
                            SlidableAction(
                              onPressed: (context) {
                                // _editKomunikasi(context, listUmum) TODO: edit form
                              },
                              icon: Icons.edit,
                            ...
                            SlidableAction(
                              onPressed: (context) {
                                // _deleteKomunikasi(context, listUmum) TODO: delete selected list
                              },
                              icon: Icons.delete,
                        ...
                        child: buildListTile(listTahfidz))
                    : buildListTile(listTahfidz),
                const Divider(
                  thickness: 0.1,
                ),
              ],
  ...
  Widget buildListTile(KomunikasiTahfidzModel listTahfidz) {
    return ListTile(
      onTap: () {
        // TODO: navigation to detail/view page
      },
      title: Text(listTahfidz.jenisHafalan ?? ''),
      subtitle: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(listTahfidz.namaPembimbing ?? ''),
          Text(listTahfidz.catatan ?? ''),
        ],
      ),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(listTahfidz.tanggal ?? ''),
