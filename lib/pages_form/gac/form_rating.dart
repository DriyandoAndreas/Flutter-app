...
class FormRatingGac extends StatefulWidget {
  const FormRatingGac({super.key});

  @override
  State<FormRatingGac> createState() => _FormRatingGacState();
}

class _FormRatingGacState extends State<FormRatingGac> {
        ...
        child: Padding(
          ...
          child: Consumer<SchoolRatingProvider>(
            builder: (context, value, child) {
              rateMap['fasilitas'] ??=
                  double.parse(value.rating?.review?.fasilitas ?? '0');
              rateMap['pelayanan'] ??=
                  double.parse(value.rating?.review?.pelayanan ?? '0');
              rateMap['lokasi'] ??=
                  double.parse(value.rating?.review?.lokasi ?? '0');
              rateMap['kondisi_bangunan'] ??=
                  double.parse(value.rating?.review?.kondisibangunan ?? '0');
              rateMap['pengajaran'] ??=
                  double.parse(value.rating?.review?.pengajaran ?? '0');
              ...
              return Column(
                children: [
                  ...
                  buildRate('Fasilitas', rateMap['fasilitas'] / 2, 'fasilitas'),
                  const SizedBox(height: 12),
                  buildRate('Pelayanan', rateMap['pelayanan'] / 2, 'pelayanan'),
                  const SizedBox(height: 12),
                  buildRate('Lokasi', rateMap['lokasi'] / 2, 'lokasi'),
                  const SizedBox(height: 12),
                  buildRate('Kondisi Bangunan', rateMap['kondisi_bangunan'] / 2,
                      'kondisi_bangunan'),
                  const SizedBox(height: 12),
                  buildRate(
                      'Pengajaran', rateMap['pengajaran'] / 2, 'pengajaran'),
                  comments(),
                  const SizedBox(
                    height: 32,
                  ),
          ...
          onPressed: () async {
            ...
            if (npsn != null && token != null && iduser != null) {
              await service.addComments(
                ...
                npsn: npsn,
                data: rateMap,
                komentar: iscommented ? commented.text : komentar.text,
              );
